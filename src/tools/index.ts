...
import { buildDelegateToSubagent, formatDelegateToSubagentResult } from "./general/delegate-to-subagent"
...
export function buildTools(model: LanguageModel): {
  tools: ToolSet
  toolFormatters: Record<string, ToolResultFormatter>
} {
  const tools: ToolSet = {
  ...
  tools.delegate_to_subagent = buildDelegateToSubagent(model, tools)
  ...
  const toolFormatters: Record<string, ToolResultFormatter> = {
    ...
    delegate_to_subagent: formatDelegateToSubagentResult,
  }

  return { tools, toolFormatters }
}
